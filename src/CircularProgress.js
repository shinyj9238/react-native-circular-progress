...
export default class CircularProgress extends React.Component {
    ...
    render() {
        const {size, width, tintColor, backgroundColor, style, rotation, linecap, children, circleDegree} = this.props;
        const backgroundPath = this.circlePath(size / 2, size / 2, size / 2 - width / 2, 0, circleDegree);

        const fill = this.extractFill(this.props.fill);
        const circlePath = this.circlePath(size / 2, size / 2, size / 2 - width / 2, 0, circleDegree * fill / 100);
